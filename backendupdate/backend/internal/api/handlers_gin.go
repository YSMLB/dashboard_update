...
func (h *GinHandler) RefreshData(c *gin.Context) {
	...
	// Перед ручным refresh подтягиваем свежие файлы с 1С в конвертер.
	// Это важно для Docker: при пересборке/перезапуске контейнера папка конвертера может быть пустой.
	if cfg, err := config.Load(); err == nil {
		...
		log.Printf("[API] Предупреждение: не удалось загрузить конфиг для SyncFromOneC: %v", err)
	}
	...
	// Важно: после обновления JSON надо сбросить кэш сверки,
	// чтобы /attendance/reconcile/* не отдавал устаревшие (пустые) результаты.
	h.reconciliation.InvalidateCache()
