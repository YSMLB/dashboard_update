...
type ScheduleGridRecord struct {
	...
	IsRemote     bool   `json:"isRemote"`     // Дистант: true если в ячейке стоит прочерк вместо аудитории
}
				...
				for group, colIdx := range groupCols {
					...
					if discipline != "" {
						records = append(records, ScheduleGridRecord{
							...
							Teacher:      teacher,
							Location:     location,
							IsRemote:     strings.Contains(strings.ToLower(location), "дистант"),
						})
						rowRecords++
					}
				}
...
func parseLessonNumber(s string) int {
	...
	// Поддерживаем варианты: "1", "1-я", "1 пара", "Пара 1", "№1", "1. ..." и т.п.
	re := regexp.MustCompile(`\b(\d{1,2})\b`)
	m := re.FindStringSubmatch(s)
	if len(m) != 2 {
		// fallback на начало строки
		var num int
		if _, err := fmt.Sscanf(s, "%d", &num); err == nil && num > 0 && num <= 10 {
			return num
		}
		return 0
	}
...
func parseLessonCell(cellValue string) (discipline, teacher, location string) {
	...
	// Если в ячейке есть прочерк "—" / "-" (обычно вместо аудитории) — считаем, что это дистант.
	// Для совместимости оставляем это в Location как "Дистант".
	for _, l := range nonEmptyLines {
		if l == "-" || l == "—" || l == "–" {
			// Не выходим сразу — дисциплина/препод могут быть выше строкой.
			if location == "" {
				location = "Дистант"
			}
		}
