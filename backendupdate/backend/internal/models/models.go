...
type AttendanceRecordJSON struct {
	...
	LessonNumber int    `json:"lessonNumber,omitempty"` // Номер пары (1-6), если удалось определить при парсинге
	Discipline   string `json:"discipline,omitempty"`   // Дисциплина из ведомости (fallback для привязки к паре)
}
