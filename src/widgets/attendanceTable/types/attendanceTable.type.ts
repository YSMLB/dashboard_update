...
export interface AttendanceTableProps extends CardProps {
  ...
  /**
   * Отключает подсветку «будущих» пар и текст «Пара ещё не началась».
   * Нужен для исторического режима, чтобы старые даты показывались как есть.
   */
  disableFutureHighlight?: boolean;
}
