...
function normalizeKey(value: string): string {
  ...
  // Для сравнения кодов групп/отделений отбрасываем всё кроме букв/цифр.
  // В выгрузках встречаются разные варианты скобок/дефисов/пробелов — они не должны ломать матч.
  return normalized.replace(/[^0-9a-zа-я]+/gi, "");
}
...
export function GroupsLevel({ department, onSelectGroup, historyDate }: GroupsLevelProps) {
  ...
  const operationalDate = historyDate ?? today;
  const isHistoryToday = historyDate != null && historyDate === today;
  ...
  const groupAttendanceData = useMemo(() => {
    ...
    for (const grp of groups) {
      ...
      const normalizedGroup = normalizeKey(grp.group);

      for (let lessonIndex = 0; lessonIndex < 6; lessonIndex++) {
        const lessonData = lessonsData[lessonIndex];
          // Ищем группу по всем отделениям.
          // Даже если department в drill и department в reconcile немного расходятся,
          // group-код обычно уникален, поэтому лучше показать правильные цифры по группе.
          const groupData = lessonData?.byDepartment?.flatMap((d) => d.byGroup)
            ?.find((g) => normalizeKey(g.group) === normalizedGroup);
          ...
          return (
            ...
            >
              <AttendanceTable
                ...
                disableFutureHighlight={historyDate != null && !isHistoryToday}
              />
            </div>
          );
        })}
